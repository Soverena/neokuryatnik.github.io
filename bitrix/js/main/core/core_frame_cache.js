    ...
    BX.frameCache.writeCache = function(blocks) {
        for (var i = 0; i < blocks.length; i++) {
            if (blocks[i].PROPS.USE_BROWSER_STORAGE === true) {
                this.writeCacheWithID(blocks[i].ID, blocks[i].CONTENT, blocks[i].HASH, JSON.stringify(blocks[i].PROPS));
            }
        }
    }
    ;
            ...
            this.cacheDataBase.getRows({
                ...
                success: BX.proxy(function(res) {
                    if (res.items.length > 0) {
                        this.cacheDataBase.updateRows({
                            ...
                            fail: function(e) {//console.error("Update cache error: ", e);
                            }
                ...
                fail: BX.proxy(function(e) {
                    this.cacheDataBase.addRow({
                        ...
                        fail: function(error) {//console.error("Add cache error: ", error);
                        }
                    });
                }, this)
            });
    ...
    BX.frameCache.insertBanner = function() {
        ...
        BX.ready(BX.proxy(function() {
            var banner = BX.create("a", {
                props: {
                    className: "bx-composite-btn" + (BX.type.isNotEmptyString(this.vars.banner.style) ? " bx-btn-" + this.vars.banner.style : ""),
