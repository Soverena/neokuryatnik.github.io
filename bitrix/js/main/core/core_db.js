...
;(function(window) {
    ...
    BX.dataBase.create = function(params) {
        if (typeof window.openDatabase != 'undefined' || typeof window.SQLitePlugin != 'undefined' && typeof window.SQLitePlugin.openDatabase == 'function') {
        ...
        if (tableName && BX.type.isArray(fields)) {
            ...
            if (fields.length > 0) {
                for (var i = 0; i < fields.length; i++) {
                    this.jsonFields[tableName].push(fields[i].toString().toUpperCase());
                }
            } else {
    ...
    BX.dataBase.prototype.createTable = function(params) {
        ...
        this.query(this.getQuery(params)).then(function(success) {
            this.getTableList();
    ...
    BX.dataBase.prototype.getRows = function(params) {
        ...
        this.query(this.getQuery(params)).then(function(success) {
            ...
            if (this.jsonFields[tableName] && this.jsonFields[tableName].length && success.result.items.length) {
                for (var i = 0; i < success.result.items.length; i++) {
                    ...
                    for (var j = 0; j < select.length; j++) {
                        ...
                        if (typeof (select[j]) == "object") {
                            ...
                            if (field && select[j].type) {
                                if (select[j].type.toLowerCase() == 'integer' || select[j].type.toLowerCase() == 'real' || select[j].type.toLowerCase() == 'text') {
                                    field += " " + select[j].type;
                                }
                            }
        ...
        case "select":
            {
                ...
                strQuery = "SELECT " + this.getValueArrayString(select, "*") + " FROM " + tableName.toUpperCase() + " " + this.getFilter(where) + (orderParam.length > 0 ? " ORDER BY " + orderParam.join(', ') + " " : "") + (limitParam ? " LIMIT " + limitParam + " " : "");
        ...
        case "update":
            {
                ...
                values = this.getValues([set], 'update').concat(this.getValues([where]));
                break;
            }
        }
        return {
